...
namespace DudeRescueSquad.UI
{
    public class FpsDisplayer : MonoBehaviour
    {
        ...
        [Tooltip("the frequency at which the FPS counter should update (in seconds)")]
        [SerializeField] private float UpdateInterval = 0.3f;
        [Tooltip("if this is true, this counter will display a moving average of the framerate, not its instant framerate")]
        [SerializeField] private Modes Mode = Modes.Instant;
        [SerializeField] private int MinFpsValueGood = 30;
        [SerializeField] private int MinFpsValueWarning = 20;
        ...
        private const string _title = "FPS: ";
        ...
        protected virtual void Update()
        {
            ...
            if (_timeLeft <= 0.0)
            {
                ...
                if (_currentFPS >= 0 && _currentFPS <= 300)
                {
                    ...
                    {
                        case Modes.Instant:
                            _text.text = $"{_title}{_stringsFrom00To300[_currentFPS]}";
                            _text.color = GetFPSColor(_currentFPS);
                            break;
                        case Modes.MovingAverage:
                            _text.text = $"{_title}{_stringsFrom00To300[_average]}";
                            _text.color = GetFPSColor(_average);
                            break;
                        case Modes.InstantAndMovingAverage:
                            _text.text = $"{_title}{_stringsFrom00To300[_currentFPS] + " / " + _stringsFrom00To300[_average]}";
                            _text.color = GetFPSColor(_currentFPS);
                            break;
                    }
        ...
        {
            if (fps >= MinFpsValueGood) return Color.green;
            else if (fps >= MinFpsValueWarning) return Color.yellow;
