...
namespace DudeRescueSquad.Tools
{
    ...
    /// 
    /// Save : SaveLoadManager.Save(TestObject, FileName+SaveFileExtension, FolderName);
    /// 
    /// Load : TestObject = (YourObjectClass)MMSaveLoadManager.Load(typeof(YourObjectClass), FileName + SaveFileExtension, FolderName);
    /// 
    /// Delete save : SaveLoadManager.DeleteSave(FileName+SaveFileExtension, FolderName);
    /// 
    /// Delete save folder : SaveLoadManager.DeleteSaveFolder(FolderName);
    /// 
    /// You can also specify what ISaveLoadManagerMethod the system should use. By default it's binary but you can also pick binary encrypted, json, or json encrypted
    /// You'll find examples of how to set each of these in the SaveLoadTester class
    /// 
    /// </summary>
    public static class SaveLoadManager
    {
        /// the method to use when saving and loading files (has to be the same at both times of course)
        public static ISaveLoadManagerMethod saveLoadMethod = new SaveLoadManagerMethodBinary();
