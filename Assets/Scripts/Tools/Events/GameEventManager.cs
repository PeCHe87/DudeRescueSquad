...
namespace DudeRescueSquad.Tools
{
    ...
    {
        private static Dictionary<Type, List<GameEventListenerBase>> _subscribersList;

        static GameEventManager()
        {
            _subscribersList = new Dictionary<Type, List<GameEventListenerBase>>();
        }
        ...
        public static void AddListener<T>(GameEventListener<T> listener) where T : struct
        {
            Type eventType = typeof(T);

            if (!_subscribersList.ContainsKey(eventType))
                _subscribersList[eventType] = new List<GameEventListenerBase>();
        ...
        /// <param name="listener">listener.</param>
        /// <typeparam name="T">The event type.</typeparam>
        public static void RemoveListener<T>(GameEventListener<T> listener) where T : struct
        {
            Type eventType = typeof(T);
            ...
            List<GameEventListenerBase> subscriberList = _subscribersList[eventType];
            bool listenerFound;
            listenerFound = false;
        ...
        {
            List<GameEventListenerBase> list;
            ...
            {
                (list[i] as GameEventListener<T>).OnGameEvent(newEvent);
            }
        }
        ...
        private static bool SubscriptionExists(Type type, GameEventListenerBase receiver)
        {
            List<GameEventListenerBase> receivers;
    ...
    public static class GameEventRegister
    {
        ...
        public static void EventStartListening<T>(this GameEventListener<T> caller) where T : struct
        {
            GameEventManager.AddListener<T>(caller);
        }
        ...
        /// <typeparam name="T">the event Type</typeparam>
        /// <param name="caller"></param>
        public static void EventStopListening<T>(this GameEventListener<T> caller) where T : struct
        {
            GameEventManager.RemoveListener<T>(caller);
        }
    }
