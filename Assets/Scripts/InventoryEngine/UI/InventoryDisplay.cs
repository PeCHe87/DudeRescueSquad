...
namespace DudeRescueSquad.InventoryEngine
{
    ...
    /// </summary>
    public class InventoryDisplay : MonoBehaviour, GameEventListener<InventoryEvent>
    {
        ...
        /// <summary>
        /// On Enable, we start listening for InventoryEvents
        /// </summary>
        ...
        /// <summary>
        /// On Disable, we stop listening for InventoryEvents
        /// </summary>
        ...
        protected virtual void DrawSlot(int i)
        {
            if (!DrawEmptySlots)
            {
                if (InventoryItem.IsNull(TargetInventory.Content[i])) return;
            }
        ...
        protected virtual void InitializeSlotPrefab()
        {
            ...
            _slotPrefab.GetComponentNoAlloc<Image>().raycastTarget = true;

            _slotPrefab.AddComponent<InventorySlot>();
            _slotPrefab.GetComponentNoAlloc<InventorySlot>().transition = Selectable.Transition.SpriteSwap;
            ...
            _slotPrefab.GetComponentNoAlloc<InventorySlot>().interactable = true;

            _slotPrefab.AddComponent<CanvasGroup>();
            _slotPrefab.GetComponentNoAlloc<CanvasGroup>().alpha = 1;
            _slotPrefab.GetComponentNoAlloc<CanvasGroup>().interactable = true;
            _slotPrefab.GetComponentNoAlloc<CanvasGroup>().blocksRaycasts = true;
            _slotPrefab.GetComponentNoAlloc<CanvasGroup>().ignoreParentGroups = false;
        ...
        public virtual void OnGameEvent(InventoryEvent inventoryEvent)
        {
            ...
            if (!inventoryEvent.TargetInventoryName.Equals(this.TargetInventoryName)) return;
            ...
            switch (inventoryEvent.InventoryEventType)
            {
                ...
                case InventoryEventType.ItemUnEquipped:
                    this.ReturnInventoryFocus();
                    break;
