...
namespace DudeRescueSquad.InventoryEngine
{
    ...
    /// </summary>
    [Serializable]
    public class Inventory : MonoBehaviour, GameEventListener<InventoryEvent>, GameEventListener<GameEvent>
    {
        ...
        public virtual bool UseItem(InventoryItem item, int index, InventorySlot slot = null)
        {
            ...
            if (item.Use())
            {
                ...
                InventoryEvent.Trigger(InventoryEventType.ItemUsed, slot, this.name, item.Copy(), 0, index);
                RemoveItem(index, 1);
            }
        ...
        public virtual List<int> InventoryContains(string searchedName)
        {
            ...
            for (int i = 0; i < Content.Length; i++)
            {
                if (!InventoryItem.IsNull(Content[i]))
                {
                    if (Content[i].ItemID.Equals(searchedName))
                    {
                        list.Add(i);
                    }
                }
        ...
        /// <returns>A list of item matching the search criteria.</returns>
        /// <param name="searchedType">The searched type.</param>
        public virtual List<int> InventoryContains(DudeRescueSquad.InventoryEngine.ItemClasses searchedClass)
        {
            List<int> list = new List<int>();
        ...
        public virtual void OnGameEvent(InventoryEvent inventoryEvent)
        {
            ...
            if (inventoryEvent.TargetInventoryName != this.name) return;
        ...
        {
            if (gameEvent.EventName.Equals("Save") && Persistent)
            {
                SaveInventory();
            }

            if (gameEvent.EventName.Equals("Load") && Persistent)
            {
